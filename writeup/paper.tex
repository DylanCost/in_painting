...
\begin{abstract}
Image inpainting, the task of reconstructing missing or corrupted regions in images, remains a fundamental challenge in computer vision. We present a systematic comparison of three generative paradigms for facial image inpainting: Diffusion Models, Variational Autoencoders (VAEs), and Flow Matching. To ensure a fair evaluation, we employ a unified U-Net architecture across all models, identical training configurations, and consistent mask generation strategies on the CelebA dataset at 128$\times$128 resolution. Our experiments over 100 training epochs reveal that VAEs achieve superior reconstruction quality with PSNR of 33.8 dB and SSIM of 0.961, while Flow Matching provides competitive performance with greater training stability. Diffusion models, despite their recent success in generative tasks, exhibit significant training instability in this constrained setting, suggesting they may require longer training horizons or additional architectural modifications for inpainting tasks.
\end{abstract}
...
Despite the proliferation of generative approaches, direct comparisons remain challenging due to architectural differences, varying training protocols, and inconsistent evaluation settings. Prior work typically optimizes each model class independently, making it difficult to isolate the contribution of the generative paradigm from confounding factors.

In this work, we address this gap through a controlled comparison of Diffusion Models, VAEs, and Flow Matching for facial inpainting. Our experimental design emphasizes fairness through several key decisions: (1) a shared U-Net backbone architecture across all models, (2) identical data preprocessing and mask generation procedures, (3) consistent training hyperparameters and compute budgets (100 epochs), and (4) evaluation on the same held-out test set using standard metrics.

Our contributions are as follows:
\begin{itemize}
    \item We present the first systematic comparison of Diffusion, VAE, and Flow Matching paradigms for image inpainting under controlled experimental conditions.
    \item We demonstrate that VAEs achieve superior reconstruction metrics (PSNR, SSIM, MAE) on facial inpainting when architectural and training factors are held constant.
    \item We provide analysis of training dynamics, revealing that Diffusion models exhibit substantial instability in this setting, while Flow Matching offers a favorable stability-performance trade-off.
\end{itemize}
...
To isolate the effect of generative paradigm from architectural confounds, we employ a unified U-Net backbone across all three models. Our U-Net consists of four encoder and four decoder blocks with channel dimensions $[64, 128, 256, 512]$. Each block contains two convolutional layers with GroupNorm and SiLU activations. We incorporate self-attention at the $16\times16$ resolution level to capture long-range dependencies.
...
Training hyperparameters are held constant: Adam optimizer with learning rate $10^{-4}$, batch size 32, and 100 training epochs. We evaluate on the validation set every epoch and report metrics on the held-out test set using the best validation checkpoint.
...
At inference, we perform $T$ denoising steps, initializing masked regions with pure noise while preserving known pixels. The model iteratively refines the masked regions to produce coherent completions.
